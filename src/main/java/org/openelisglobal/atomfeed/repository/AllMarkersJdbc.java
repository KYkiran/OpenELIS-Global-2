...
public class AllMarkersJdbc implements AllMarkers {
    ...
    public Marker get(URI feedUri) {
        String sql = "SELECT feed_uri, last_read_entry_id, feed_uri_for_last_read_entry " +
                     "FROM " + SCHEMA + "event_records_offset_marker WHERE feed_uri = ?";
        List<Marker> markers = jdbcTemplate.query(sql,
            (rs, rowNum) -> new Marker(
                URI.create(rs.getString("feed_uri")),
                rs.getString("last_read_entry_id"),
    ...
    public List<Marker> getMarkerList() {
        return jdbcTemplate.query(
            ...
            "FROM " + SCHEMA + "event_records_offset_marker",
            (rs, rowNum) -> new Marker(
                URI.create(rs.getString("feed_uri")),
                rs.getString("last_read_entry_id"),
                rs.getString("feed_uri_for_last_read_entry") != null
