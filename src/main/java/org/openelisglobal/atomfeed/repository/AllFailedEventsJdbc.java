...
public class AllFailedEventsJdbc implements AllFailedEvents {
    ...
    private final RowMapper<FailedEvent> mapper = (rs, rowNum) -> {
        ...
        return new FailedEvent(
            rs.getString("feed_uri"),
            event,
            rs.getString("error_message"),
            failedAt != 0 ? failedAt : System.currentTimeMillis(),
            rs.getInt("retries")
        );
    };
   ...
   @Override
    public void addOrUpdate(FailedEvent event) {
        ...
        if (existing == null) {
            jdbcTemplate.update(
                ...
                event.getFeedUri(),
                event.getEventId(),
                event.getEvent().getContent(),
                event.getEvent().getTitle(),
                event.getErrorMessage(),
                event.getRetries(),
                event.getFailedAt()                  // epoch millis → BIGINT
            );
        } else {
            jdbcTemplate.update(
                "UPDATE " + SCHEMA + "failed_events " +
                "SET retries = ?, error_message = ?, failed_at = ? " +
    ...
    public int getNumberOfFailedEvents(String feedUri) {
        Integer count = jdbcTemplate.queryForObject(
            "SELECT COUNT(*) FROM " + SCHEMA + "failed_events WHERE feed_uri = ?",
            Integer.class, feedUri
        );
    ...
    public void remove(FailedEvent event) {
        jdbcTemplate.update(
            "DELETE FROM " + SCHEMA + "failed_events WHERE feed_uri = ? AND event_id = ?",
    ...
    public FailedEvent get(String feedUri, String eventId) {
        List<FailedEvent> results = jdbcTemplate.query(
            "SELECT * FROM " + SCHEMA + "failed_events WHERE feed_uri = ? AND event_id = ? LIMIT 1",
            mapper, feedUri, eventId
        );
    ...
    public List<FailedEvent> getFailedEvents(String feedUri) {
        return jdbcTemplate.query(
            "SELECT * FROM " + SCHEMA + "failed_events WHERE feed_uri = ?",
            mapper, feedUri
        );
    }
    ...
    public FailedEvent getByEventId(String eventId) {
        List<FailedEvent> list = jdbcTemplate.query(
            "SELECT * FROM " + SCHEMA + "failed_events WHERE event_id = ?",
            mapper, eventId
        );
