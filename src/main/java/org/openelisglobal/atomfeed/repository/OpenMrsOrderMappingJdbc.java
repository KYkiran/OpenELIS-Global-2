...
public class OpenMrsOrderMappingJdbc {
    ...
    public void insert(String encounterUuid, String orderUuid, String oe2PatientId, String status, String errorMessage) {
        jdbcTemplate.update(
                "INSERT INTO " + SCHEMA + "openmrs_order_mapping "
                        + "(openmrs_encounter_uuid, openmrs_order_uuid, oe2_patient_id, status, error_message) "
                        + "VALUES (?, ?, ?, ?, ?) "
                        + "ON CONFLICT (openmrs_order_uuid) DO UPDATE SET "
                        + "status = EXCLUDED.status, error_message = EXCLUDED.error_message, "
                        + "oe2_patient_id = EXCLUDED.oe2_patient_id, updated_at = NOW()",
                encounterUuid, orderUuid, oe2PatientId, status, errorMessage);
    }
    ...
    public boolean exists(String orderUuid) {
        Integer count = jdbcTemplate.queryForObject(
                "SELECT COUNT(*) FROM " + SCHEMA + "openmrs_order_mapping WHERE openmrs_order_uuid = ?",
    ...
    public String getStatus(String orderUuid) {
        try {
            return jdbcTemplate.queryForObject(
                    "SELECT status FROM " + SCHEMA + "openmrs_order_mapping WHERE openmrs_order_uuid = ?",
